...
def call_openrouter(messages: list, model_idx: int = 0) -> tuple:
    ...
    headers = {
        ...
        "HTTP-Referer": "https://erp-cairo-agent.local",
...
# ----------------------------------------------------------------------
#  CLI MODE (fallback when Tkinter missing)
# ----------------------------------------------------------------------
class CLIAgent:
    def __init__(self):
        self.conversation = [{"role": "system", "content": SYSTEM_PROMPT}]
        print("\n" + "="*60)
        print("🇪🇬  ERP Cairo Lead Agent (CLI Mode)  🇪🇬")
        print("="*60)
        print("I help you find ERP-ready companies in Cairo, Egypt.\n")
        print("Type 'exit' or 'quit' to stop.\n")
        self._show_help()
    
    def _show_help(self):
        print("Quick examples:")
        print("  - Find 5 manufacturing companies in Cairo that use ERP")
        print("  - Show me logistics companies needing ERP in Cairo")
        print("  - List retail companies in Cairo planning to implement SAP\n")
    ...
    def run(self):
        while True:
            try:
                ...
                print("\n⏳ Searching for ERP leads in Cairo...")
...
# ----------------------------------------------------------------------
#  GUI MODE (original, unchanged logic)
# ----------------------------------------------------------------------
if TKINTER_AVAILABLE:
    ...
    # Start GUI if Tkinter is available
    ...
    # Fallback to CLI when Tkinter missing
    if __name__ == "__main__":
        print("\n⚠️  Tkinter not found or not configured. Starting in text-only (CLI) mode.\n")
        print("To use the full GUI, install python3-tk (Linux) or use a local Python environment.\n")
        cli = CLIAgent()
        cli.run()
